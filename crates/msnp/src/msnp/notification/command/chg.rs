...
impl TryFromRawCommand for Chg {
    ...
    fn try_from_raw(raw: RawCommand) -> Result<Self, Self::Err> {
        ...
        let raw_presence_status = split.pop_front().ok_or(CommandError::MissingArgument(
            raw.command.clone(),
            "presence_status".into(),
            2,
        ))?;
        let presence_status = PresenceStatus::from_str(&raw_presence_status)?;

        let raw_capabilities = split.pop_front().ok_or(CommandError::MissingArgument(
            raw.command.clone(),
        ...
        let raw_avatar_msn_obj = split.pop_front().ok_or(CommandError::MissingArgument(
            raw.command.clone(),
            "avatar_msn_obj".into(),
            4,
        ))?;
...
impl Display for Chg {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "CHG {tr_id} {presence_status} {client_capabilities} ",
            tr_id = self.tr_id,
