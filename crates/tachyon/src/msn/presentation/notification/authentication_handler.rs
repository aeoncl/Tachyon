...
pub(crate) async fn handle_auth(command: NotificationClientCommand, notif_sender: Sender<NotificationServerCommand>, tachyon_state: &AppState, local_store: &mut LocalClientData, config: &TachyonConfig, matrix_login_service: &Box<dyn MatrixLoginService>) -> Result<(), anyhow::Error> {
    match command {
