...
use crate::msn::domain::msn_bridge::{BridgeConfiguration, MsnpBridge};
use crate::msn::presentation::notification::command_handler::handle_command;
use crate::msn::presentation::notification::local_client_data::LocalClientData;
...
impl NotificationServer {
    ...
    pub async fn listen(ip_addr: &str, port: u32, global_shutdown_recv: Receiver<()>, global_state: AppState, bridge_configuration: BridgeConfiguration) -> Result<(), anyhow::Error>{
            ...
            loop {
                ...
                tokio::select! {
                    accepted = listener.accept() => {
                        ...
                        let bridge_config_clone = bridge_configuration.clone();
                        let _result = tokio::spawn(async move {
                            handle_client(socket, global_shutdown_recv_clone.resubscribe(), global_state_clone, bridge_config_clone).await
                        });
                    }
...
async fn handle_client(socket: TcpStream, mut global_shutdown_recv: broadcast::Receiver<()>, global_state: AppState, bridge_configuration: BridgeConfiguration) -> Result<(), anyhow::Error> {
    ...
    let bridge = global_state.msnp_bridges().add_bridge(MsnpBridge::new(bridge_configuration));
                ...
                match bytes_read {
                    ...
                    Ok(bytes_read) => {
                        ...
                        match commands {
                            ...
                            Ok(commands) => {
                                handle_commands(commands, &command_sender, &global_state, &mut local_client_data, bridge.clone()).await;
                            }
                        }
...
async fn handle_commands(commands: Vec<RawCommand>, command_sender: &Sender<NotificationServerCommand>, global_state: &AppState, local_client_data: &mut LocalClientData, bridge: MsnpBridge) {
    for command in commands {
        ...
        match notification_command {
            ...
            Ok(notification_command) => {
                let command_result = handle_command(notification_command, command_sender.clone(), &global_state, local_client_data, &global_state.get_config(), bridge.clone()).await;
