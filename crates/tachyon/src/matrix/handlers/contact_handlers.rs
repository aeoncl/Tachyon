...
pub(super) async fn handle_contacts(
    ...
    println!(
        "Handling contact event for room: {}",
        room.room_id().to_string()
    );

    let mut contacts = compute_contacts(&event, &room, &tachyon_client.users())
        .await
        .unwrap();
    ...
    if !contacts.is_empty() {
        ...
        for contact in contacts.drain(..) {
            contact_service.push_delta(contact);
        }
    }
}
...
) -> Result<Vec<AddressBookContact>, anyhow::Error> {
    let mut out = Vec::new();
...
pub(super) async fn handle_contacts_stripped(
    ...
    let mut contacts = compute_contacts_from_stripped_event(&event, &room, &tachyon_client.users())
        .await
        .unwrap();
...
async fn compute_contacts_from_stripped_event(
    event: &StrippedRoomMemberEvent,
    room: &Room,
    user_service: &Arc<dyn UserService>,
) -> Result<Vec<AddressBookContact>, anyhow::Error> {
...
pub async fn compute_all_contacts(
    client: Client,
    user_service: Arc<dyn UserService>,
) -> Vec<AddressBookContact> {
    let mut out = Vec::new();
            ...
            .await
        {
            match event {
                RawSyncOrStrippedState::Sync(sync) => {
                    ...
                    let mut memberships = compute_contacts(&deserialized, &room, &user_service)
                        .await
                        .unwrap();
                ...
                RawSyncOrStrippedState::Stripped(stripped) => {
                    ...
                    let mut stripped_memberships =
                        compute_contacts_from_stripped_event(&deserialized, &room, &user_service)
                            .await
                            .unwrap();
...
pub(super) async fn handle_tombstone(
    ...
    tachyon_client
        .contacts()
        .push_delta(AddressBookContact::Contact(contact));
}
