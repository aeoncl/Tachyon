...
pub(super) async fn handle_memberships(
    ...
    let is_space = room
        .room_type()
        .is_some_and(|room_type| matches!(room_type, RoomType::Space));

    if is_space {
        return;
    }

    let mut members = compute_memberships(&event, &room, &tachyon_client.users())
        .await
        .unwrap();

    if !members.is_empty() {
        tachyon_client.contacts().push_membership_deltas(members);
    }
}

async fn compute_memberships(
    event: &SyncRoomMemberEvent,
    room: &Room,
    ...
    if event_is_about_me {
        let room_msn_user = user_service
        ...
        match event.membership() {
            MembershipState::Ban => {
                let delete_allow_member = BaseMember::new_passport_member(
                    &room_msn_user,
                ...
                let delete_pending_member = BaseMember::new_passport_member(
                    &room_msn_user,
                    MemberState::Accepted,
                    RoleList::Pending,
                    true,
                ...
                let delete_reverse_member = BaseMember::new_passport_member(
                    &room_msn_user,
                    MemberState::Accepted,
                    RoleList::Reverse,
                    true,
            ...
            MembershipState::Join => {
                let allow_member = BaseMember::new_passport_member(
                    &room_msn_user,
                    MemberState::Accepted,
                    RoleList::Allow,
                    false,
                ...
                let reverse_member = BaseMember::new_passport_member(
                    &room_msn_user,
                    MemberState::Accepted,
                    RoleList::Reverse,
                    false,
...
pub(super) async fn handle_memberships_stripped(
    ...
    let mut members =
        compute_memberships_from_stripped_event(&event, &room, &tachyon_client.users())
            .await
            .unwrap();
...
async fn compute_memberships_from_stripped_event(
    ...
    if event_is_about_me {
        ...
        match room.state() {
            RoomState::Invited => {
                // I'm invited
                let invite_member = BaseMember::new_invite_passsport_member(
                    &room_msn_user,
...
pub async fn compute_all_memberships(
    client: Client,
    user_service: &Arc<dyn UserService>,
) -> Vec<BaseMember> {
    let mut out = Vec::new();
    ...
    for room in client.rooms() {
        if let Ok(Some(event)) = room
            .get_state_event_static_for_key::<RoomMemberEventContent, _>(room.own_user_id())
            .await
        {
            match event {
                RawSyncOrStrippedState::Sync(sync) => {
                    let deserialized = sync.deserialize().unwrap();
                    let mut memberships = compute_memberships(&deserialized, &room, user_service)
                        .await
                        .unwrap();
                ...
                RawSyncOrStrippedState::Stripped(stripped) => {
                    ...
                    let mut stripped_memberships =
                        compute_memberships_from_stripped_event(&deserialized, &room, user_service)
                            .await
                            .unwrap();
...
pub(super) async fn handle_tombstone(
    ...
    let contact_service = tachyon_client.contacts();
    contact_service.push_membership_delta(delete_allow_member);
    contact_service.push_membership_delta(delete_pending_member);
    contact_service.push_membership_delta(delete_reverse_member);
}
