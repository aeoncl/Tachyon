...
use crate::matrix::directs::{get_invite_room_mapping_info, get_joined_room_mapping_info, RoomMappingInfo};
...
pub async fn handle_memberships(client: Client, response: SyncResponse, mut client_data: ClientData, notif_sender: Sender<NotificationServerCommand>) -> Result<(), anyhow::Error> {
        ...
        for state_event in &update.state {
             match state_event.deserialize() {
                 Ok(AnySyncStateEvent::RoomMember(room_member_event)) => {
                     error!("Received join room member event : {:?}", room_member_event);

                     match room_member_event {
                         SyncRoomMemberEvent::Original(og_rm_event) => {
                             let mapping = get_joined_room_mapping_info(&room, me, &client).await?;
                 ...
                 Ok(other) => {
                     error!("Received join non member event : {:?}", other);

                 }
                 Err(e) => {
                     error!("Couldnt deserialize join sync state event: {:?}", e);
                 },
    ...
    for (room_id, update) in response.rooms.invite {
        ...
        for state_event in update.invite_state.events {
            if let Ok(AnyStrippedStateEvent::RoomMember(stripped_rm_event)) = state_event.deserialize() {
                match stripped_rm_event.content.membership {
                    MembershipState::Invite => {

                        if stripped_rm_event.state_key == me {

                            let target_user = stripped_rm_event.sender.as_ref();
                            let mapping = get_invite_room_mapping_info(&room_id, target_user, &stripped_rm_event, &client).await?;
                            ...
                            match mapping {
                                RoomMappingInfo::Direct(direct_target) => {
                                    ...
                                    log::info!("AB - I received Direct invite from: {}", &target_msn_addr);
                                ...
                                RoomMappingInfo::Group => {
                                    log::info!("AB - I received a Group invite from: {}", &target_user);
    ...
    for (room_id, update) in response.rooms.leave {
        ...
        for state_event in update.state {
            ...
            if let Ok(AnySyncStateEvent::RoomMember(room_member_event)) = state_event.deserialize() {
                match room_member_event {
                    SyncRoomMemberEvent::Original(og_rm_event) => {
                        debug!("Received LEFT ROOM UPDATE MEMBER EVENT Room: {}, event:{:?}", &room_id, &og_rm_event);
                    }
                    _ => {}
                }
            }
