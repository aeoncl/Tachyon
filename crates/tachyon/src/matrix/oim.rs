...
pub async fn handle_oims(
    client: Client,
    response: SyncResponse,
    mut client_data: TachyonSessionData,
    notif_sender: Sender<NotificationServerCommand>,
    first_sync_token: Option<String>,
) -> Result<(), OIMError> {
    ...
    for (room_id, room) in &response.rooms.joined {
        ...
        if let Some(prev_batch) = room.timeline.prev_batch.as_ref() {
            ...
            if let Some(room) = client.get_room(&room_id) {
                let mut config =
                    get_message_options(first_sync_token.clone(), Some(prev_batch.clone()));
                                ...
                                AnySyncMessageLikeEvent::RoomMessage(ref room_message) => {
                                    let oim = match room_message {
                                        SyncMessageLikeEvent::Original(ref original_event) => {
                                            ...
                                            handle_original_message(
                                                &original_event.content.msgtype,
                                                &room_id,
                                                room_uuid.clone(),
                                                e.event_id(),
                                                original_event.origin_server_ts.0.into(),
                                                e.sender(),
                                                display_name,
                                                seq_num,
                                                me_email_addr.clone(),
                                        ...
                                        SyncMessageLikeEvent::Redacted(_redacted) => None,
                                    };
                ...
                match e {
                    AnySyncMessageLikeEvent::RoomMessage(ref room_message) => {
                        let oim = match room_message {
                            ...
                            SyncRoomMessageEvent::Redacted(_redacted) => None,
                        };
    ...
    let payload = if !client_data.soap_holder().oims.is_empty() {
        RawMsgPayloadFactory::get_initial_mail_data_too_large_notification()
    } else {
        RawMsgPayloadFactory::get_initial_mail_data_empty_notification()
    };

    notif_sender
        .send(NotificationServerCommand::MSG(MsgServer {
...
pub fn handle_original_message(
    ...
    Ok(match message_type {
        ...
        MessageType::Text(text) => Some(handle_text_message_event(
            &room_id,
            room_uuid,
            event_id,
            event_timestamp,
...
pub fn handle_text_message_event(
    room_id: &OwnedRoomId,
    room_uuid: Uuid,
    event_id: &EventId,
    event_timestamp: i64,
    ...
    let recv_datetime = DateTime::from_timestamp_millis(event_timestamp).ok_or(
        OIMError::EventTimestampConvertionError {
