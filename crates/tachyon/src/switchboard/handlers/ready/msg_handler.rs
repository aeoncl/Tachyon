...
use msnp::shared::payload::msg::chunked_msg_payload::{
    ChunkMetadata, ChunkedMsgPayload, MsgChunks,
};
use tokio::sync::mpsc::Sender;

pub(super) async fn handle_msg(
    msg_command: MsgClient,
    ...
    if let MsgPayload::Chunked(chunk) = msg_command.payload {
        if let Some(complete) = handle_chunked(chunk, local_switchboard_data).await? {
            handle_msg_payload_task(
                msg_command.tr_id,
                msg_command.ack_type,
                complete,
                &room,
                &command_sender,
            );
...
pub async fn handle_chunked(
    chunk: ChunkedMsgPayload,
    local_switchboard_data: &mut LocalSwitchboardData,
) -> Result<Option<MsgPayload>, anyhow::Error> {
    ...
    match local_switchboard_data.chunks.remove(&message_id) {
        ...
        None => {
            local_switchboard_data
                .chunks
                .insert(message_id, MsgChunks::from_first_chunk(chunk)?);
        }
    }
...
pub fn handle_msg_payload_task(
    tr_id: u128,
            ...
            MsgPayload::TextPlain(text_plain) => {
                ...
                if let Err(e) = room_clone.send_with_dedup(message).await {
                    ...
                    match ack_type {
                        ...
                        | MsgAcknowledgment::AckA
                        | MsgAcknowledgment::AckD => {
                            command_sender_clone
                                .send(SwitchboardServerCommand::NAK(NakServer::new(tr_id)))
                                .await;
                        }
                        _ => {}
                    }
                    ...
                    match ack_type {
                        MsgAcknowledgment::AckA | MsgAcknowledgment::AckD => {
                            command_sender_clone
                                .send(SwitchboardServerCommand::ACK(AckServer::new(tr_id)))
                                .await;
                        }
                        _ => {}
                    }
