...
use crate::tachyon::services::session::user_service::{room_to_msn_user, UserService};
...
pub(crate) async fn handle_auth(
    command: SwitchboardClientCommand,
    command_sender: Sender<SwitchboardServerCommand>,
    tachyon_state: &GlobalState,
    local_switchboard_data: &mut LocalSwitchboardData,
) -> Result<(), anyhow::Error> {
    match command {
                ...
                Some(tachyon_client) => {
                    ...
                    match matrix_client.get_room(token.room_id.as_ref()) {
                        ...
                        Some(room) => {
                            ...
                            local_switchboard_data.email_addr =
                                room_msn_user.get_email_address().clone();
                            local_switchboard_data.endpoint_guid =
                                room_msn_user.endpoint_id.endpoint_guid.clone();
                            ...
                            let mut initial_roster = if ROOM_USER_PORTAL_MODE {
                                ...
                                get_initial_roster_with_room_user(&room, room_msn_user.clone())
                                    .await?
                            };
                            ...
                            command_sender
                                .send(SwitchboardServerCommand::OK(ans_command.get_ok_response()))
                                .await?;
                            ...
                            let mut switchboard_handle = tachyon_client
                                .switchboards()
                                .get_mut(&*token.room_id)
                                .expect("To be here");
                            //set handle as ready
                            switchboard_handle
                                .set_state(SwitchboardState::Ready {
        ...
        SwitchboardClientCommand::USR(usr_command) => {
            ...
            match tachyon_state.tachyon_clients().get(token.as_str()) {
                ...
                Some(tachyon_client) => {
                    ...
                    local_switchboard_data.endpoint_guid =
                        usr_command.endpoint_id.endpoint_guid.clone();
                    ...
                    let _ = command_sender
                        .send(SwitchboardServerCommand::USR(
                            usr_command.get_ok_response_for(email.to_string()),
                        ))
                        .await;
                }
            }
...
pub(crate) async fn handle_init(
    ...
    match command {
        ...
        SwitchboardClientCommand::CAL(cal_client) => {
            ...
            let _ = command_sender
                .send(SwitchboardServerCommand::CAL(CalServer {
            ...
            if email == local_switchboard_data.email_addr {
                ...
                if let Some(room) = maybe_found {
                    let target_room_user = user_service
                    ...
                    let switchboard_handle = SwitchboardHandle::new_ready(
                        local_switchboard_data.session_id.clone(),
                        room.room_id().to_owned(),
                        command_sender.clone(),
                    );
                    tachyon_client
                        .switchboards()
                        .insert(switchboard_handle.room_id.clone(), switchboard_handle);
...
async fn get_initial_roster_with_room_user(
    room: &Room,
    room_msn_user: MsnUser,
) -> Result<Vec<MsnUser>, anyhow::Error> {
    let mut out = get_initial_roster(room).await?;
    out.push(room_msn_user);
    Ok(out)
}

async fn send_initial_roster_member(
    tr_id: u128,
    index: u32,
    count: u32,
    member: MsnUser,
    ...
    command_sender
        .send(SwitchboardServerCommand::IRO(IroServer::new(
            tr_id,
            index,
            count,
...
async fn send_initial_joined_member(
    member: MsnUser,
    command_sender: &Sender<SwitchboardServerCommand>,
) -> Result<(), anyhow::Error> {
    command_sender
        .send(SwitchboardServerCommand::JOI(JoiServer {
