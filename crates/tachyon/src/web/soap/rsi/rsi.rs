...
pub async fn rsi(
    ...
    let soap_action = headers
        .get("SOAPAction")
        .ok_or(RSIError::MissingHeader("SOAPAction".into()))?
        .to_str()?
        .trim_start_matches("\"")
        .trim_end_matches("\"");
    ...
    let token = TicketToken(
        header_env
            .header
            .ok_or(RSIError::AuthenticationFailed {
                source: anyhow!("Missing Soap Header",),
                service_url: "https://rsi.hotmail.com/rsi/rsi.asmx".to_string(),
            })?
            .passport_cookie
            .t,
    );
    ...
    let tachyon_client =
        state
            ...
            .get(token.as_str())
            .ok_or(RSIError::AuthenticationFailed {
                source: anyhow!("Missing Tachyon Client"),
                service_url: "https://rsi.hotmail.com/rsi/rsi.asmx".to_string(),
            })?;
    ...
    match soap_action {
        "http://www.hotmail.msn.com/ws/2004/09/oim/rsi/GetMessage" => {
            get_message(
                GetMessageMessageSoapEnvelope::try_from_xml(&body)?,
                token,
                client,
                &tachyon_client,
            )
        ...
        "http://www.hotmail.msn.com/ws/2004/09/oim/rsi/GetMetadata" => {
            get_metadata(
                GetMetadataMessageSoapEnvelope::try_from_xml(&body)?,
                token,
                client,
                &tachyon_client,
            )
        ...
        "http://www.hotmail.msn.com/ws/2004/09/oim/rsi/DeleteMessages" => {
            delete_messages(
                DeleteMessagesSoapEnvelope::try_from_xml(&body)?,
                token,
                client,
                &tachyon_client,
            )
        ...
        _ => Err(RSIError::UnsupportedSoapAction(soap_action.to_string())),
    }
}
