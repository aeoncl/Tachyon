...
pub(super) async fn ab_contact_update(
    request: AbcontactUpdateMessageSoapEnvelope,
    _token: TicketToken,
    ...
    let mut contacts = request
        ...
        .body
        .contacts
        .ok_or(anyhow!("Invalid contacts"))
        .map(|c| c.contact)?;

    if contacts.len() != 1 {
        return Err(ABError::InternalServerError(anyhow!(
            "Only one contact can be deleted at a time"
        )));
    }
    ...
    let contact = contacts
        .drain(..)
        .next()
        .ok_or(anyhow!("No contacts to delete"))?;

    let contact_info = contact
        .contact_info
        .ok_or(ABError::InternalServerError(anyhow!(
    ...
    let contact = {
        let contact_list = tachyon_client
            .get_contact_list()
            .lock()
            .map_err(|e| anyhow!("Could not mutex lock contact list {}", e))?;
        match contact_list.find_contact_by_uuid(&contact_uuid) {
            Some(c) => c.clone(),
            None => {
                return Ok(shared::build_soap_response(
                    SoapFaultResponseEnvelope::new_contact_doesnt_exist(soap_action, &contact_uuid)
                        .to_xml()?,
                    StatusCode::OK,
                ));
            }
        ...
        if !messenger_user {
            match user_service.find_room_from_email(&contact.email_address)? {
                Some(room) => {
                    room.leave().await?;
                    let soap_body =
                        AbcontactUpdateResponseMessageSoapEnvelope::get_response(&cache_key);
                    return Ok(shared::build_soap_response(
                        soap_body.to_xml()?,
                        StatusCode::OK,
                    ));
                ...
                None => {
                    return Ok(shared::build_soap_response(
                        SoapFaultResponseEnvelope::new_contact_doesnt_exist(
                            soap_action,
                            &contact_uuid,
                        )
                        .to_xml()?,
