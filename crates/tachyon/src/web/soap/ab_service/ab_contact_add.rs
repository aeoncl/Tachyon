                 ...
                 if let Some(Ok(contact_email)) = contact_info.passport_name.map(|p| EmailAddress::from_str(&p)) {
                     ...
                     match client.get_dm_room(&contact_user_id) {
                         None => {
                             ...
                             return Ok(contact_create(&contact_email, &cache_key, tachyon_client).await?);
                         }
                         Some(dm) => {
...
async fn contact_create(contact_email_addr: &EmailAddress, cache_key: &str, tachyon_client: TachyonClient) -> Result<Response, anyhow::Error> {
    let contact_uuid = contact_email_addr.to_uuid();

    let contact_email_addr_clone = contact_email_addr.clone();
    task::spawn(async move {
        sleep(Duration::from_secs(3)).await;
        delete_user_contact(contact_email_addr_clone, tachyon_client)
    });
