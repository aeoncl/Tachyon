...
pub async fn get_verification_poll(
    ...
    let refresh_url = format!(
        ...
        notification_id, flow_id, user_id_raw
    );
    let refresh_url_with_state = format!(
        "{}&state={}",
        refresh_url,
        state_name(&verification_request.state())
    );
