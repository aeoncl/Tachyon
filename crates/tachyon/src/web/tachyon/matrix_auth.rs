...
pub async fn post_auth(
    ...
    let login_successful = if let Ok((matrix_token, _)) = state
        .matrix_login_service()
        .login_with_password(&matrix_id, password, !state.get_config().strict_ssl)
        .await
    {
        let ticket_token = TicketToken(
            state
                ...
                .map_err(|e| RST2Error::InternalServerError {
