...
use crate::web::tachyon::confirm_device::{other_device, recover, reset_identity};
use axum::body::Body;
...
pub fn tachyon_router(state: GlobalState) -> Router<GlobalState> {
    Router::new()
        ...
        .route(
            "/confirm_device/reset_identity",
            post(reset_identity::post_reset_identity),
        )
        .route(
            "/confirm_device/reset_identity",
            get(reset_identity::get_reset_identity),
        )
        .route("/confirm_device/recover", get(recover::get_recover))
        .route("/confirm_device/recover", post(recover::post_recover))
        .route(
            "/confirm_device/other_device",
            get(other_device::get_other_device),
        )
        .route(
            "/confirm_device/other_device",
            post(other_device::post_other_device),
        )
        .route("/verification", get(verification::get_verification_poll))
        .route(
            "/verification/sas_v1/{action}",
            post(verification::sas_v1_actions::post_sas_v1_action),
        )
