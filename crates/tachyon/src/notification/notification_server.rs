...
use msnp::msnp::{
    notification::command::command::NotificationServerCommand, raw_command_parser::RawCommandParser,
};
...
async fn handle_client(
    socket: TcpStream,
    mut global_shutdown_recv: broadcast::Receiver<()>,
    global_state: GlobalState,
) -> Result<(), anyhow::Error> {
    ...
    let mut local_client_data =
        LocalClientData::new(client_shutdown_snd.clone(), client_shutdown_recv);
        ...
        match notification_command {
            ...
            Ok(notification_command) => {
                let command_result = handle_command(
                    ...
                    &global_state,
                    local_client_data,
                    &global_state.get_config(),
                )
                .await;

                if let Err(error) = command_result {
                    error!(
                        "MSNP|NS: An error has occured handling a notification command: {}",
                        &error
                    );
                    debug!("MSNP|NS: {:?}", &error);
...
) -> Sender<NotificationServerCommand> {
