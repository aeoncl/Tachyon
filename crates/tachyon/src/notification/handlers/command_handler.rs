...
pub(crate) async fn handle_command(
    ...
    local_client_data: &mut LocalClientData,
    ...
    let _command_result = match &local_client_data.phase {
        ...
        ConnectionPhase::Authenticating => {
            auth::handle_auth(
                ...
                local_client_data,
                config,
                global_state.matrix_login_service(),
            )
            .await
        }
        ConnectionPhase::Ready => {
            let tachyon_client = local_client_data
                .tachyon_client
                .as_ref()
                .ok_or(anyhow!("Tachyon Client should be here by now"))?
                .clone();
            handle_ready(
                command,
                command_sender,
...
mod tests {
    ...
    impl MatrixLoginService for TestMatrixLoginService {
        ...
        async fn login_with_password(
            ...
            password: &str,
            disable_ssl: bool,
        ) -> Result<(AccessToken, Client), TachyonError> {
            let test = self
                .client
                .matrix_auth()
                .login_username(matrix_id.to_string(), password)
                .await
                .unwrap();
    ...
    async fn handshake_happy_flow_test() {
        ...
        handle_command(cvr, snd, &state, &mut local_client_data, state.get_config())
            .await
            .unwrap();
    ...
    async fn auth_i_test() {
        ...
        let usr_i = NotificationClientCommand::USR(UsrClient {
            tr_id: 1,
            auth_type: AuthOperationTypeClient::Sso(SsoPhaseClient::I {
                email_addr: EmailAddress::from_str("aeon@test.com").unwrap(),
            }),
        });
        ...
        assert!(matches!(
            usr_resp.auth_type,
            OperationTypeServer::Sso(SsoPhaseServer::S { .. })
        ));

        let OperationTypeServer::Sso(SsoPhaseServer::S { policy, nonce }) = usr_resp.auth_type
        else {
