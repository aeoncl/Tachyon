...
use msnp::msnp::notification::command::usr::{
    AuthOperationTypeClient, AuthPolicy, OperationTypeServer, SsoPhaseClient, SsoPhaseServer,
    UsrServer,
};
...
pub(crate) async fn handle_auth(
    command: NotificationClientCommand,
                ...
                AuthOperationTypeClient::Sso(content) => {
                    match content {
                        SsoPhaseClient::I { email_addr } => {
                            ...
                            let gcf_response = RawCommand::with_payload(
                                "GCF 0",
                                SHIELDS_PAYLOAD.as_bytes().to_vec(),
                            );

                            notif_sender
                                .send(NotificationServerCommand::USR(usr_response))
                                .await?;
                            notif_sender
                                .send(NotificationServerCommand::RAW(gcf_response))
                                .await?;
                        }
                        ...
                        SsoPhaseClient::S {
                            ...
                            let matrix_token = tachyon_state
                                .secret_encryptor()
                                .decrypt(ticket_token.as_str())?;

                            let matrix_client = matrix_login_service
                                .login_with_token(&user_id, &matrix_token, !config.strict_ssl)
                                .await?;

                            let endpoint_id = EndpointId::new(
                                local_store.email_addr.clone(),
                                Some(endpoint_guid),
                            );
                            let msn_user = MsnUser::new(endpoint_id);

                            let session_data = TachyonSessionData::new(
                                matrix_client.clone(),
                                config.clone(),
                                msn_user.clone(),
                                ticket_token.clone(),
                                NotificationHandle::new(notif_sender.clone()),
                                local_store.client_shutdown_snd.clone(),
                                local_store.client_shutdown_recv.resubscribe(),
                            );
                            let tachyon_client = TachyonClient::from_session_data(session_data);
                            let drop_guard = tachyon_state.insert_clients(
                                ticket_token.as_str().to_owned(),
                            ...
                            let usr_response = UsrServer::new(
                                command.tr_id,
...
fn sync_with_server_task(
    notif_sender: &Sender<NotificationServerCommand>,
    local_store: &LocalClientData,
    ...
    task::spawn(async move {
        let cross_signed = check_device_is_crossed_signed(&matrix_client_clone)
            .await
            .unwrap();

        if !cross_signed {
            let sign_sync_loop_kill_snd = cross_signing::cross_sign_sync_task(
                &matrix_client_clone,
                client_shutdown_recv.resubscribe(),
            )
            .await
            .unwrap();
            ...
            let verif_not = NotificationServerCommand::NOT(NotServer {
                payload: NotificationPayloadType::Normal(NotificationFactory::alert(
                    ...
                    format!("http://127.0.0.1:{}/tachyon", config_clone.http_port).as_str(),
                    format!(
                        "http://127.0.0.1:{}/tachyon/confirm_device?t={}",
                        config_clone.http_port,
                        &ticket_token_clone.as_str()
                    )
                    .as_str(),
                    format!(
                        "http://127.0.0.1:{}/tachyon/confirm_device?t={}",
        ...
        notif_sender_clone
            .send(NotificationServerCommand::RAW(RawCommand::without_payload(
                "SBS 0 null",
            )))
            .await;
        ...
        let initial_mail_data = NotificationServerCommand::MSG(MsgServer {
            sender: "Hotmail".to_string(),
            display_name: DisplayName::new_from_ref("Hotmail"),
            payload: MsgPayload::Raw(
                RawMsgPayloadFactory::get_initial_mail_data_empty_notification(),
            ),
        });
