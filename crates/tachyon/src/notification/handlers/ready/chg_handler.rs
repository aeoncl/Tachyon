...
pub async fn handle_chg(
    ...
    command_sender
        .send(NotificationServerCommand::CHG(command.clone()))
        .await?;
    ...
    if local_store.needs_initial_presence {
        ...
        tokio::spawn(async move {
            ...
            for contact in contacts {
                ...
                let display_name = if let Some(proxy_user) = user_service
                    .resolve_room_proxy_user_from_email(&contact.email_address)
                    .await
                {
                ...
                let _ = command_sender
                    .send(NotificationServerCommand::ILN(IlnServer {
                        tr_id: command.tr_id,
                        presence_status: PresenceStatus::NLN,
                        target_user: network_id_email.clone(),
                        via: None,
                        display_name: display_name
                            .map(|name| DisplayName::new(name))
                            .unwrap_or_default(),
                ...
                let _ = command_sender
                    .send(NotificationServerCommand::UBX(UbxServer {
