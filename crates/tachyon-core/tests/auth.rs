...
use tachyon_core::domain::auth::{BridgeMetadata, Credential, InteractiveAuthStarted, BridgeLinkToken};
...
impl Fixture {
    ...
    fn new(session: Arc<FakeBackendSession>, stored: bool) -> Self {
        let account_repository = if stored {
            AccountRepositoryInMem::with_login(&BridgeLinkToken::new("tachyon-token"), &LoginId::new("login-1"))
        } else {
    ...
    async fn sign_in_pending_details(&self) -> (Step, String) {
    ...
    fn wait(&self) -> Waiting {
        ...
        tokio::spawn(async move { use_case.wait_until_session_ready(&token).await })
    }
...
async fn signing_in_with_a_verified_device_opens_the_session() {
    ...
    assert!(fixture.use_case.ready_session(&fixture.token).is_some());
...
async fn signing_in_with_an_unverified_device_asks_for_verification_and_withholds_the_session() {
    ...
    assert!(fixture.use_case.ready_session(&fixture.token).is_none());
...
#[tokio::test]
async fn two_concurrent_sign_ins_let_one_through_and_refuse_the_other() {
    let fixture = Fixture::with_stored_login([DeviceStatus::Verified]);

    let outcomes = tokio::join!(fixture.sign_in(), fixture.sign_in());

    let outcomes = [outcomes.0, outcomes.1];
    assert_eq!(
        outcomes.iter().filter(|outcome| matches!(outcome, Ok(SignIn::Ready(_)))).count(),
        1
    );
    assert_eq!(
        outcomes.iter().filter(|outcome| matches!(outcome, Err(AuthError::AlreadySignedIn))).count(),
        1
    );
    assert_eq!(fixture.auth_service.restore_calls(), 1);
}
...
async fn signing_in_without_a_stored_login_starts_an_interactive_one() {
    ...
    assert!(fixture.use_case.has_login(&fixture.token));
    assert!(matches!(
        fixture.use_case.prompt_for_interactive_flow(flow_id),
...
async fn finishing_the_browser_login_binds_the_token_and_reports_the_device() {
    ...
    assert_eq!(
        finished.redirect_url.as_deref(),
    ...
    assert!(fixture.use_case.has_login(&fixture.token));
    assert!(fixture.use_case.ready_session(&fixture.token).is_none());
    assert!(fixture.use_case.prompt_for_interactive_flow(&flow_id).is_none());
}
...
async fn abandoning_releases_a_waiter_parked_on_the_browser_login() {
    ...
    fixture.use_case.abandon_login(&fixture.token).await.unwrap();
...
#[tokio::test]
async fn a_second_sign_in_while_one_is_pending_is_refused() {
    let fixture = Fixture::without_stored_login([DeviceStatus::Verified]);
    fixture.sign_in_pending().await;
    let waiting = fixture.wait();

    let refused = fixture.sign_in().await;

    assert!(matches!(refused, Err(AuthError::AlreadySignedIn)));
    assert_eq!(fixture.session.discard_calls(), 0, "the first sign-in is untouched");
    assert_eq!(fixture.auth_service.start_calls(), 1);
    assert!(still_waiting(&waiting).await);
}

#[tokio::test]
async fn a_second_sign_in_on_a_ready_login_is_refused_until_it_is_abandoned() {
    let fixture = Fixture::with_stored_login([DeviceStatus::Verified]);
    fixture.sign_in_ready().await;
...
async fn a_pending_sign_in_says_where_the_browser_must_go() {
    ...
    let (_, login_url) = interactive.sign_in_pending_details().await;
    let (_, confirm_url) = verification.sign_in_pending_details().await;
...
async fn finishing_with_a_password_binds_the_token_like_the_browser_callback() {
    ...
    let finished = fixture
        .use_case
        .finish_interactive_flow(&flow_id, Credential::Password(Password::new("hunter2")))
        .await
        .unwrap();

    assert_eq!(finished.device_status, DeviceStatus::Verified);
    assert!(finished.redirect_url.is_none(), "nothing left for the browser to do");
    assert_eq!(fixture.session.authenticate_calls(), 1);
    assert!(fixture.stored_login().await.is_some());
    assert!(fixture.use_case.ready_session(&fixture.token).is_some());
}
...
async fn a_rejected_credential_leaves_the_login_pending_for_another_try() {
    ...
    let refused = fixture
        .use_case
        .finish_interactive_flow(&flow_id, Credential::Password(Password::new("wrong")))
        .await;

    assert!(matches!(refused, Err(AuthError::BackendError(_))));
    assert!(fixture.use_case.has_login(&fixture.token));
    assert!(fixture.use_case.prompt_for_interactive_flow(&flow_id).is_some());
    assert_eq!(fixture.session.close_calls(), 0);
...
async fn a_callback_for_a_replaced_login_binds_nothing_in_the_store() {
    ...
    let finishing = fixture.finish_in_background(&flow_id(&step), &stale).await;
...
async fn a_stale_callback_leaves_the_replacement_sign_in_alone() {
    ...
    assert!(fixture.use_case.prompt_for_interactive_flow(&second_flow).is_some());
...
async fn forgetting_a_live_login_discards_it_and_drops_its_row() {
    ...
    fixture.use_case.hard_logout(&fixture.token).await.unwrap();
...
async fn sweeping_forgets_unbound_logins_and_prunes_the_backend_to_the_rest() {
    let store = AccountRepositoryInMem::with_login(&BridgeLinkToken::new("tachyon-token"), &LoginId::new("login-1"))
