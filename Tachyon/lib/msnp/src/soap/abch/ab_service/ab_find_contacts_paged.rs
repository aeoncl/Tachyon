...
pub mod response {
    ...
    impl AbfindContactsPagedResponseMessageSoapEnvelope {
        pub fn new(uuid: Uuid, cache_key: &str, msn_addr: &str, display_name: &str, contacts: Vec<ContactType>, profile_update: bool) -> Self {
            ...
            let result = AbfindContactsPagedResultType{ groups: Some(groups), contacts: Some(array_of_contact), circle_result, ab };
            let body_body = AbfindContactsPagedResponse{ ab_find_contacts_paged_result: result };
            let body = SoapAbfindContactsPagedResponseMessage{ body: body_body, fault: None };
